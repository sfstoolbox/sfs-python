...
order = 2  # order of image sources
...
xs, walls = sfs.util.image_sources_for_box(x0[0:2], L[0:2], order)
source_strength = np.prod(coeffs[0:4]**walls, axis=1)
...
P = sfs.mono.source.point_image_sources(omega, x0, [1, 0, 0], grid, L, order, coeffs=coeffs)
...
p = sfs.time.source.point_image_sources(x0, signal, 0.004, grid, L, order, coeffs=coeffs)
